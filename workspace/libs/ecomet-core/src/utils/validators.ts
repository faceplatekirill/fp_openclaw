...
export function validateArchives(archives: unknown): string[] {
...
export function validateTimestampMs(value: unknown, name: string): number {
  if (typeof value !== 'number') {
    throw new EcometError(
      `'${name}' must be a timestamp in milliseconds, got: ${String(value)}`,
  ...
  if (!Number.isInteger(value)) {
    throw new EcometError(
      `'${name}' must be an integer timestamp in milliseconds, got: ${String(value)}`,
      ErrorCode.INVALID_PARAMS,
    );
  }
  ...
  if (value <= MIN_MS_TIMESTAMP) {
    throw new EcometError(
      `'${name}' appears to be in seconds (got ${value}), expected milliseconds — multiply by 1000`,
      ErrorCode.INVALID_PARAMS,
    );
  }
...
export function validateTimestamps(timestamps: unknown): number[] {
  ...
  const validated = timestamps.map((value, index) =>
    validateTimestampMs(value, `timestamps[${index}]`),
  );
  ...
  for (let index = 1; index < validated.length; index += 1) {
    ...
    if (current <= previous) {
      throw new EcometError(
        `timestamps must be monotonically increasing, but timestamps[${index}] (${current}) <= timestamps[${index - 1}] (${previous})`,
        ErrorCode.INVALID_PARAMS,
      );
    }
  }
