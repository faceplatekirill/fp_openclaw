...
export const M2_EVAL_FIXTURES: M2EvalFixture[] = [
  {
    id: 'RF-1',
    type: 'routing',
    ...
    expected_behavior:
      'Route to the deterministic alarm-summary skill so total alarms and alarms-per-hour are computed in code.',
    anti_pattern:
      'Treat an alarm-rate question as either a raw alarm listing or a measured-value aggregate request.',
  },
  {
    id: 'RF-2',
    type: 'routing',
    ...
    expected_behavior:
      'Use the alarm-summary path so source grouping, counts, and percentages are computed deterministically.',
    anti_pattern:
      'Return raw alarms or object browsing results instead of a ranked offender list.',
  },
  {
    id: 'RF-3',
    type: 'routing',
    ...
    expected_behavior:
      'Keep row-oriented retrieval on scada-alarm-list so filters and pagination semantics stay canonical.',
    anti_pattern:
      'Route a listing request to the alarm-summary analytics surface.',
  },
  {
    id: 'RF-4',
    type: 'routing',
    ...
    expected_behavior:
      'Route to alarm-summary so flood windows are detected with the defined threshold and time window.',
    anti_pattern:
      'Answer a flood question with a plain alarm list or an unrelated quality diagnostic.',
  },
  {
    id: 'RF-5',
    type: 'routing',
    ...
    expected_behavior:
      'Use alarm-summary so active alarms are deduplicated by source and surfaced with earliest onset facts.',
    anti_pattern:
      'Reduce a standing-alarm request to a raw active=true alarm listing.',
  },
  {
    id: 'RF-6',
    type: 'routing',
    ...
    expected_behavior:
      'Use alarm-summary so repeated alarm sources are counted and ranked as chattering candidates.',
    anti_pattern:
      'Return raw alarm rows and leave chattering detection to the model.',
  },
  {
    id: 'RF-7',
    type: 'routing',
    ...
    expected_behavior:
      'Use the export skill after a data-producing skill returns JSON so file generation stays deterministic.',
    anti_pattern:
      'Have the model improvise CSV text or treat export as part of a data-retrieval skill.',
  },
  {
    id: 'RF-8',
    type: 'routing',
    ...
    expected_behavior:
      'Use the canonical alarm-summary skill for period-scoped alarm analytics requests.',
    anti_pattern:
      'Ignore the summary intent and return a plain list of alarm rows.',
  },
  {
    id: 'OF-1',
    ...
    expected_behavior:
      'Return one alarm_summary block with all six metric families, distributions, warnings, provenance, and completeness.',
    anti_pattern:
      'Return ad hoc markdown or omit required summary metrics from the structured output.',
  },
  {
    id: 'OF-2',
    type: 'output_contract',
    ...
    expected_behavior:
      'Render chat output with the alarm summary heading, all metric sections, and a provenance footer.',
    anti_pattern:
      'Drop one of the metric sections or answer with free-form prose that is not tied to the canonical renderer.',
  },
  {
    id: 'OF-3',
    type: 'output_contract',
    ...
    expected_behavior:
      'Return a scope_view with metadata.export_path, csv metadata, and a real file containing headers, rows, and provenance.',
    anti_pattern:
      'Inline CSV content into the response or omit the export path metadata.',
  },
  {
    id: 'EF-1',
    type: 'edge_case',
    ...
    expected_behavior:
      'Return zeroed metrics, empty arrays, complete status, and an empty_result warning instead of throwing.',
    anti_pattern:
      'Treat no alarms as an error or return null metrics.',
  },
  {
    id: 'EF-2',
    type: 'edge_case',
    ...
    expected_behavior:
      'Return one total alarm, a positive hourly rate, one top offender at 100%, no flood, and no chattering alarms.',
    anti_pattern:
      'Divide by zero, produce percentages above 100, or misclassify one alarm as a flood or chatter.',
  },
  {
    id: 'EF-3',
    type: 'edge_case',
    ...
    expected_behavior:
      'Stop at 10,000 alarms, mark completeness partial, emit a safety_limit warning, and report fetched-count metadata.',
    anti_pattern:
      'Fetch unbounded alarm pages or claim the result is complete after truncation.',
  },
  {
    id: 'EF-4',
    type: 'edge_case',
    ...
    expected_behavior:
      'Split the time range into <=30-day windows, merge and dedupe alarms, and emit a window_split warning.',
    anti_pattern:
      'Attempt one oversized alarm query or silently ignore part of the requested range.',
  },
  {
    id: 'EF-5',
    type: 'edge_case',
    ...
    expected_behavior:
      'Honor the explicit timezone in provenance and compute the summary over the resolved real time range without crashing.',
    anti_pattern:
      'Ignore the requested timezone or produce malformed provenance around the DST boundary.',
  },
  {
    id: 'EF-6',
    type: 'edge_case',
    ...
    expected_behavior:
      'Create a CSV with headers plus warnings and provenance, and return the export file metadata normally.',
    anti_pattern:
      'Fail export because the source block has no rows or invent a fake data row.',
  },
  {
    id: 'EF-7',
    type: 'edge_case',
    ...
    expected_behavior:
      'Serialize alarm-summary overview, offenders, flood periods, standing alarms, and chattering sections into one CSV.',
    anti_pattern:
      'Flatten the summary into an unreadable blob or drop the multi-section structure.',
  },
  {
    id: 'EF-8',
    type: 'edge_case',
    ...
    expected_behavior:
      'Normalize supported alias forms into canonical scope, time, and options without rejecting the request.',
    anti_pattern:
      'Fail a retry payload that only uses documented alias keys.',
  },
  {
    id: 'EF-9',
    type: 'edge_case',
    ...
    expected_behavior:
      'Fetch active alarms over a separate 30-day lookback or reuse the main fetch only when the main range already covers that window.',
    anti_pattern:
      'Compute standing alarms only from the user-requested short main range and miss older active alarms.',
  },
];
