...
export const M0_EVAL_FIXTURES: EvalFixture[] = [
  {
    id: 'RF-1',
    type: 'routing',
    ...
    expected_behavior:
      'Route to scada-alarm-summary so the skill can compute alarm frequency KPIs from alarm records.',
    anti_pattern:
      'Treat the question as a field aggregate request and answer it via archive statistics alone.',
  },
  {
    id: 'RF-2',
    type: 'routing',
    ...
    expected_behavior:
      'Route first to scada-period-aggregates to find the peak, then use scada-point-history to locate when it occurred.',
    anti_pattern:
      'Route directly to raw history and skip the aggregate-first composition that identifies the peak.',
  },
  {
    id: 'RF-3',
    type: 'routing',
    ...
    expected_behavior:
      'Route to scada-object-explore to discover patterns, types, and candidate objects in scope.',
    anti_pattern:
      'Treat discovery as an archive or alarm question instead of returning topology metadata.',
  },
  {
    id: 'DB-1',
    ...
    expected_behavior:
      'The supervisor routes to scada-alarm-summary, and that skill may internally delegate to alarm-analyst with a restricted subset.',
    anti_pattern:
      'Let the supervisor delegate directly to alarm-analyst or compute alarm KPIs inline without the stable skill surface.',
  },
  {
    id: 'DB-2',
    type: 'delegation_boundary',
    ...
    expected_behavior:
      'Keep orchestration in main-supervisor under the scada-incident-review surface, composing alarms and trends directly.',
    anti_pattern:
      'Delegate the whole incident review to alarm-analyst, which only owns alarm KPI workflows.',
  },
  {
    id: 'EF-1',
    type: 'edge_case',
    ...
    expected_behavior:
      'When total exceeds returned results, mark completeness as partial, preserve total counts, and provide continuation guidance plus a warning.',
    anti_pattern:
      'Present the returned page as if it were the full alarm result set or drop the total count.',
  },
  {
    id: 'EF-2',
    type: 'edge_case',
    ...
    expected_behavior:
      'Preserve invalid and unresolved tags separately from null values and surface warnings that explain why data is missing.',
    anti_pattern:
      'Collapse invalid or unresolved tags into nulls or silently drop them from the snapshot output.',
  },
  {
    id: 'EF-3',
    type: 'edge_case',
    ...
    expected_behavior:
      'Keep provenance.timezone explicit, preserve wall-clock day boundaries through the DST transition, and document any 23-hour or 25-hour bucket caveat.',
    anti_pattern:
      'Bucket data in UTC without disclosure or silently lose data around the DST boundary.',
  },
  {
    id: 'EF-4',
    type: 'edge_case',
    ...
    expected_behavior:
      'Split the request into windows of 30 days or less, merge results, dedupe boundary overlaps, and report completeness for the merged response.',
    anti_pattern:
      'Send a single 45-day query, silently truncate the period, or omit the fact that the result was merged from multiple windows.',
  },
  {
    id: 'EF-5',
    type: 'edge_case',
    ...
    expected_behavior:
      'Return needs_clarification behavior and ask which fields define running, stopped, and fault states instead of inferring them.',
    anti_pattern:
      'Guess state mappings from field names and produce a fabricated availability report.',
  },
  {
    id: 'EF-6',
    type: 'edge_case',
    ...
    expected_behavior:
      'Return the chat-facing summary from scada-alarm-summary, keep any visible truncation explicit, and preserve export as a downstream option instead of the primary route.',
    anti_pattern:
      'Treat chat summarization as a spreadsheet-export request or pretend the visible top slice proves the whole result is small.',
  },
];
