...
export async function searchObjects(
  ...
  const query =
    `get ${normalized.select.join(', ')} from 'project' where ${whereClause} page ${from}:${to} format $to_json`;
...
function validateParams(params: SearchParams): NormalizedSearchParams {
...
function normalizePatterns(value: SearchParams['pattern']): string[] {
...
function normalizeFields(value: SearchParams['fields']): Record<string, FieldValue> | undefined {
  if (value === undefined) {
    return undefined;
  }
  ...
  const entries = Object.entries(value)
    .map(([field, entryValue]) => [field.trim(), entryValue] as const)
...
): NormalizedSearchParams['search'] | undefined {
  if (value === undefined) {
    return undefined;
  }
...
function normalizeSelect(value: SearchParams['select']): string[] {
...
function normalizeLimit(value: SearchParams['limit']): number {
  if (value === undefined) {
    return 100;
  }
  if (
...
function normalizeOffset(value: SearchParams['offset']): number {
  if (value === undefined) {
    return 0;
  }
  if (
...
function buildConditions(params: NormalizedSearchParams, registry: IndexRegistry): BuiltConditions {
  const baseIndexed: string[] = [];
  const fieldIndexed: string[] = [];
  const searchIndexed: string[] = [];
  const fieldStrict: string[] = [];
  const searchStrict: string[] = [];
  ...
  if (params.patterns.length === 1) {
    ...
    const parts = params.patterns.map(
      (pattern) => `.pattern = $oid('${escapeLiteral(pattern)}')`,
    );
  ...
  if (params.fields) {
    for (const [fieldName, value] of Object.entries(params.fields)) {
      ...
      const indexedCheck = canUseIndexedOperator(
        fieldName,
        params.patterns,
        registry,
        'simple',
...
function buildSearchCondition(
  search: NonNullable<NormalizedSearchParams['search']>,
  patterns: string[],
  registry: IndexRegistry,
): SearchConditionBuild {
...
function canUseIndexedOperator(
  fieldName: string,
  patterns: string[],
  registry: IndexRegistry,
  indexKind: 'simple' | '3gram',
): { canUseIndexed: boolean; warning?: string } {
