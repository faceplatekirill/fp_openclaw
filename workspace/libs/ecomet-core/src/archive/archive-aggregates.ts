...
function validateAggregates(aggregates: unknown): AggregateSpec[] {
  ...
  for (let index = 0; index < aggregates.length; index += 1) {
    ...
    if (!isValidAggregateFn(functionName)) {
      throw new EcometError(
        `invalid aggregate function '${functionName}' at index ${index}: must be a built-in (avg, min, max, integral, standard_deviation) or 'module:function' format`,
        ErrorCode.INVALID_PARAMS,
      );
    }
...
export async function getAggregates(
  ...
  if (typeof response !== 'object' || Array.isArray(response)) {
    throw new EcometError(
      `get_aggregates returned unexpected response type: ${typeof response}`,
      ErrorCode.QUERY_FAILED,
      { response },
    );
  }

  return response as GetAggregatesResult;
}
