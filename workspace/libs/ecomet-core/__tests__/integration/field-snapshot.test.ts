...
import { EcometClient, fieldSnapshot, getSnapshot, readArchives, resolveArchives } from '../../dist/index.js';
...
async function findUnresolvedField(client: EcometClient, objectPath: string): Promise<string> {
  const probes = ['.name', '.pattern', '.folder', '__TASK_009_UNRESOLVED_FIELD__'];
...
function isSnapshotValue(value: unknown): value is number | null {
  return typeof value === 'number' || value === null;
}

async function runIntegrationTests() {
  console.log('\nField Snapshot Integration Tests\n');
  ...
  console.log('Test 1: Single tag snapshot returns number|null in values and empty unresolved/invalid');
  try {
    ...
    if (
      isSnapshotValue(result.values[key]) &&
      result.unresolved.length === 0 &&
      result.invalid.length === 0
    ) {
      console.log(`PASS Test 1: value=${String(result.values[key])}\n`);
      passed += 1;
    } else {
      console.log('FAIL Test 1: expected value key with number|null and unresolved=[] and invalid=[]\n');
      failed += 1;
    }
  } catch (error) {
  ...
  console.log('Test 2: Multiple tags snapshot returns all keys with number|null values');
  try {
    const result = await fieldSnapshot(client, {
    ...
    const allPresent = candidates.every((candidate) => {
      const key = `${candidate.object}:${candidate.field}`;
      return hasOwnProperty(result.values, key) && isSnapshotValue(result.values[key]);
    });

    if (allPresent && result.unresolved.length === 0 && result.invalid.length === 0) {
      console.log(`PASS Test 2: validated ${candidates.length} key(s)\n`);
      passed += 1;
    } else {
      console.log('FAIL Test 2: expected all candidate keys in values and unresolved=[] and invalid=[]\n');
      failed += 1;
    }
  } catch (error) {
  ...
  console.log('Test 3: Unresolved field appears in unresolved and not in values');
  try {
    ...
    if (
      ...
      console.log('FAIL Test 3: expected key in unresolved, absent from values, and invalid=[]\n');
      failed += 1;
    }
  } catch (error) {
  ...
  console.log('Test 4: Invalid object appears in invalid and not in values');
  try {
    const invalidObject = `/root/FP/PROJECT/__TASK_011_INVALID_${Date.now()}__`;
    ...
    if (
      ...
      console.log('FAIL Test 4: expected invalid key in invalid[] and unresolved=[]\n');
      failed += 1;
    }
  } catch (error) {
  ...
  console.log('Test 5: Mixed resolved and unresolved tags split correctly');
  try {
    ...
    if (
      hasOwnProperty(result.values, resolvedKey) &&
      isSnapshotValue(result.values[resolvedKey]) &&
      !hasOwnProperty(result.values, unresolvedKey) &&
      ...
      console.log('FAIL Test 5: expected resolved in values, unresolved in unresolved[], and invalid=[]\n');
      failed += 1;
    }
  } catch (error) {
  ...
  console.log('Test 6: Round-trip consistency with resolveArchives + getSnapshot');
  try {
    const key = `${primary.object}:${primary.field}`;
    ...
    const manualSnapshot = await getSnapshot(client, {
      archives: [archivePath],
      timestamp,
    });
    const rawExpected = manualSnapshot[archivePath];
    const expectedValue = typeof rawExpected === 'number' || rawExpected === null ? rawExpected : null;
    assert.strictEqual(snapshotResult.values[key], expectedValue);

    console.log(`PASS Test 6: value=${String(expectedValue)}\n`);
    passed += 1;
  } catch (error) {
  ...
  console.log('Test 7: Mixed resolved, unresolved, and invalid tags split into all buckets');
  try {
    const invalidObject = `/root/FP/PROJECT/__TASK_011_INVALID_${Date.now()}__`;
    const resolvedKey = `${primary.object}:${primary.field}`;
    const unresolvedKey = `${primary.object}:${unresolvedField}`;
    ...
    if (
      ...
      console.log('FAIL Test 7: expected resolved+unresolved+invalid three-way split\n');
      failed += 1;
    }
  } catch (error) {
