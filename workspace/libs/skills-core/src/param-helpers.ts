...
export function rejectUnexpectedKeys(
  ...
  if (unknown.length > 0) {
    ...
    throw new Error(
      `Unexpected parameter${unknown.length > 1 ? 's' : ''}: ${unknown.join(', ')}. ` +
...
function normalizeDateTime(value: unknown): unknown {
  ...
  if (offsetMatch) {
    throw new Error(
      `Do not pass offset-bearing ISO timestamps like "${trimmed}". ` +
  ...
  if (safeMatch) return `${safeMatch[1]} ${safeMatch[2]}`;
...
function normalizeAgo(
  ago: unknown,
): { amount: number; unit: string } {
  if (!isRecord(ago)) {
    throw new Error(
      'Malformed time params: ago must be an object with { amount, unit }.',
    );
  }
  ...
  if (!Number.isInteger(amount) || amount <= 0) {
    throw new Error(
      'Malformed time params: ago.amount must be a positive integer.',
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Malformed time params: ago.unit must be one of ${[...RELATIVE_TIME_POINT_UNITS].join(', ')}.`,
    );
  }
 ...
 */
function normalizeTimeObject(
  time: Record<string, unknown>,
): Record<string, unknown> {
  const unknownInner = Object.keys(time).filter(
    (key) => !KNOWN_TIME_INNER_KEYS.has(key),
  );
  if (unknownInner.length > 0) {
    throw new Error(
      `Unexpected key${unknownInner.length > 1 ? 's' : ''} inside time: ${unknownInner.join(', ')}. ` +
        'Known time keys: preset, from, to, timezone (range) or at, ago, timezone (point).',
    );
  }

  const result = { ...time };
  if ('from' in result) result.from = normalizeDateTime(result.from);
  if ('to' in result) result.to = normalizeDateTime(result.to);
  if ('at' in result) result.at = normalizeDateTime(result.at);
  if ('ago' in result) result.ago = normalizeAgo(result.ago);
  return result;
}

// ---------------------------------------------------------------------------
// Time extraction (range-based skills: history, aggregates, alarms, quality)
// ---------------------------------------------------------------------------

type SupportedRangePreset =
  | 'last_15_minutes'
  | 'last_30_minutes'
  | 'last_1_hour'
  | 'last_2_hours'
...
const NOW_RANGE_PRESETS: Record<string, SupportedRangePreset> = {
...
const FLAT_TIME_POINT_ALIASES = ['at', 'timestamp', 'timestamp_text', 'timestamp_local', 'timezone', 'timestamp_timezone', 'ago', 'offset'] as const;
const FLAT_TAG_ALIASES = ['tag', 'tag_paths', 'object', 'objects', 'field', 'label', 'unit', 'functions'] as const;

function normalizeDurationPreset(
  amount: number,
  unit: 'minute' | 'hour' | 'day',
): SupportedRangePreset | undefined {
  if (unit === 'minute') {
    if (amount === 15) return 'last_15_minutes';
    if (amount === 30) return 'last_30_minutes';
    if (amount === 60) return 'last_1_hour';
    if (amount === 120) return 'last_2_hours';
    if (amount === 360) return 'last_6_hours';
    if (amount === 1440) return 'last_24_hours';
    return undefined;
  }

  if (unit === 'hour') {
    if (amount === 1) return 'last_1_hour';
    if (amount === 2) return 'last_2_hours';
    if (amount === 6) return 'last_6_hours';
    if (amount === 24) return 'last_24_hours';
    return undefined;
  }
...
function normalizeRangeAlias(
  ...
  const match = /^(?:last)?(\d+)(minutes?|mins?|min|m|hours?|hrs?|hr|h|days?|day|d)$/.exec(
    compact,
  );
  ...
  throw new Error(
    `Unrecognized ${contextLabel} "${String(value)}". ` +
...
function rollingPresetToAgo(
  preset: SupportedRangePreset,
): { amount: number; unit: 'minute' | 'hour' | 'day' } | undefined {
  switch (preset) {
...
function parseRelativeTimePointString(
  value: string,
): { amount: number; unit: 'minute' | 'hour' | 'day' } | undefined {
  const trimmed = value.trim();
  if (trimmed.length === 0) {
  ...
  const directMatch =
    /^(?:now)?-(\d+)(minutes?|mins?|min|m|hours?|hrs?|hr|h|days?|day|d)$/.exec(
      compact,
    ) ??
...
function toTimePointFromString(
  value: string,
): { at: unknown } | { ago: { amount: number; unit: string } } {
...
function normalizeNestedTimeRangeObject(
  time: Record<string, unknown>,
): Record<string, unknown> {
  if (time.kind !== undefined || time.unit !== undefined || time.value !== undefined || time.amount !== undefined || time.count !== undefined) {
    const conflicts = ['from', 'to', 'preset', 'range', 'period'].filter(
      (key) => time[key] !== undefined,
    );
    if (conflicts.length > 0) {
      throw new Error(
        `Conflicting nested time params: structured rolling windows cannot be combined with ${conflicts.join(', ')}.`,
      );
    }

    const preset = normalizeStructuredPeriod(time);
    const normalized: Record<string, unknown> = { preset };
  ...
  const unknownInner = Object.keys(time).filter(
    (key) => !knownTimeRangeKeys.has(key),
  );
  if (unknownInner.length > 0) {
    throw new Error(
      `Unexpected key${unknownInner.length > 1 ? 's' : ''} inside time: ${unknownInner.join(', ')}. ` +
  ...
  if (time.range !== undefined) {
    const conflictingKeys = ['from', 'to', 'preset', 'period'].filter(
      (key) => time[key] !== undefined,
    );
    if (conflictingKeys.length > 0) {
      throw new Error(
        `Conflicting nested time params: range cannot be combined with ${conflictingKeys.join(', ')}.`,
      );
    }
    ...
    if (!preset) {
      throw new Error(
        'Malformed time params: time.range must be a supported rolling range string.',
      );
    }
  ...
  if (time.period !== undefined) {
    const conflictingKeys = ['from', 'to', 'preset'].filter(
      (key) => time[key] !== undefined,
    );
    if (conflictingKeys.length > 0) {
      throw new Error(
        `Conflicting nested time params: period cannot be combined with ${conflictingKeys.join(', ')}.`,
      );
    }

    let preset: SupportedRangePreset | undefined;
    if (typeof time.period === 'string') {
      preset = normalizeRangeAlias(time.period, 'time.period') ?? undefined;
      if (!preset) {
        throw new Error(
          'Malformed time params: time.period must be a supported rolling range string.',
        );
      }
      ...
      throw new Error(
        'Malformed time params: time.period must be a string like "24h" or a structured { kind: "last", value/amount, unit } object.',
      );
    }
...
function normalizeStructuredPeriod(
  period: unknown,
): SupportedRangePreset | undefined {
  if (!isRecord(period)) {
    return undefined;
  ...
  if (kind !== 'last') {
    throw new Error(
      `Unrecognized period kind "${String(period.kind)}". Structured period only supports { kind: "last", value/amount, unit }.`,
    );
  }

  if (!Number.isInteger(value) || value <= 0) {
    throw new Error(
      'Structured period requires a positive integer value.',
    );
  }

  let normalizedUnit: 'minute' | 'hour' | 'day';
  if (unit === 'minute' || unit === 'minutes' || unit === 'min' || unit === 'mins' || unit === 'm') {
    ...
    throw new Error(
      `Unrecognized structured period unit "${String(period.unit)}".`,
    );
  }
  ...
  throw new Error(
    `Unsupported structured period { kind: "last", value: ${value}, unit: "${String(period.unit)}" }. ` +
      'Use a supported rolling range such as 15 minutes, 1 hour, 6 hours, 24 hours, or 7 days.',
  );
}

function normalizeRelativeNowToken(value: unknown): string | undefined {
  if (typeof value !== 'string') {
    return undefined;
  }
...
function extractRelativeNowPreset(
  from: unknown,
  to: unknown,
  timezone: unknown,
  ...
  if (typeof from === 'number' || typeof to === 'number') {
    throw new Error(
      'Do not pre-compute epoch timestamps. Pass rolling ranges as time: { preset: "last_24_hours" } or local datetime strings with timezone.',
    );
  }
  ...
  const time: Record<string, unknown> = { preset };
...
function extractStructuredTimeRangeAlias(
  value: unknown,
): Record<string, unknown> | undefined {
  if (!isRecord(value)) {
    return undefined;
  ...
  const kind =
    typeof nestedRange.kind === 'string'
      ? nestedRange.kind.trim().toLowerCase()
      : '';

  if (kind === 'relative') {
    const fromToken =
      typeof nestedRange.from === 'string'
    ...
    const rawUnit =
      typeof nestedRange.unit === 'string'
        ? nestedRange.unit.trim().toLowerCase()
        : '';
    const fromOffset =
      typeof nestedRange.from === 'number' ? nestedRange.from : Number.NaN;
    const toOffset =
      typeof nestedRange.to === 'number' ? nestedRange.to : Number.NaN;
  ...
  const rawUnit =
    typeof nestedRange.from.unit === 'string'
      ? nestedRange.from.unit.trim().toLowerCase()
      : '';
  const amount =
    typeof nestedRange.from.value === 'number'
...
function rejectConflictingAliases(
  ...
  if (conflicts.length > 0) {
    const hint = usageHint ? ' ' + usageHint : '';
    throw new Error(
      `Conflicting params: "${nestedKey}" is present, but flat alias${conflicts.length > 1 ? 'es' : ''} ` +
...
export function extractTimeRange(
  ...
  if (isRecord(params.time)) {
    ...
    return normalizeNestedTimeRangeObject(params.time as Record<string, unknown>);
  }
  ...
  if (typeof params.time === 'string') {
    ...
    if (preset) {
      const time: Record<string, unknown> = { preset };
      if (typeof params.timezone === 'string') time.timezone = params.timezone;
      return time;
    }
  }
  ...
  if (isRecord(params.timeRange)) {
    ...
    if (structured) {
      if (typeof params.timezone === 'string') structured.timezone = params.timezone;
      return structured;
    }
  }
  ...
  if (typeof params.period === 'string') {
    ...
    const hint = usageHint ? ' ' + usageHint : '';
    throw new Error(
      `Unrecognized period "${params.period}". ` +
  ...
  const strayTimeKeys = ['time', 'timeRange', 'from', 'to', 'until', 'time_from', 'time_to', 'time_range', 'time_window', 'time_preset', 'timezone', 'preset', 'period', 'range'].filter(
    (key) => params[key] !== undefined,
  );
  if (strayTimeKeys.length > 0) {
    const hint = usageHint ? ' ' + usageHint : '';
    throw new Error(
      `Malformed time params: ${strayTimeKeys.join(', ')} present but no valid time shape matched. ` +
 ...
 */
export function extractTimePoint(
  params: Record<string, unknown>,
  usageHint?: string,
): Record<string, unknown> | undefined {
  if (isRecord(params.time)) {
    rejectConflictingAliases(params, 'time', FLAT_TIME_POINT_ALIASES, usageHint);
    const normalized = normalizeTimeObject(params.time as Record<string, unknown>);

    if (normalized.from !== undefined || normalized.to !== undefined) {
      const hint = usageHint ? ' ' + usageHint : '';
      throw new Error(
        'Exact time point skills do not accept time ranges. Pass time: { at: "YYYY-MM-DD HH:MM", timezone: "..." } or time: { ago: { amount: 1, unit: "hour" } }.' +
          hint,
      );
    }
    ...
    if (normalized.preset !== undefined) {
      const preset =
        normalizeRangeAlias(normalized.preset, 'time.preset') ?? normalized.preset;
      if (preset === 'today' || preset === 'yesterday') {
        const hint = usageHint ? ' ' + usageHint : '';
        throw new Error(
          `Exact time point skills do not accept calendar presets like "${String(normalized.preset)}". ` +
            'Use an exact local datetime or a relative ago offset instead.' +
            hint,
        );
      }

      const ago = rollingPresetToAgo(preset as SupportedRangePreset);
      if (!ago) {
        const hint = usageHint ? ' ' + usageHint : '';
        throw new Error(
          `Unrecognized time point preset "${String(normalized.preset)}". ` +
            'Use time: { at: "YYYY-MM-DD HH:MM", timezone: "..." } or time: { ago: { amount: 1, unit: "hour" } }.' +
            hint,
        );
      }

      const time: Record<string, unknown> = { ago };
      if (typeof normalized.timezone === 'string') time.timezone = normalized.timezone;
      return time;
    }
  ...
  if (typeof params.time === 'string') {
    const conflictingKeys = ['at', 'timestamp', 'timestamp_text', 'timestamp_local', 'ago', 'offset'].filter(
      (key) => params[key] !== undefined,
    );
    if (conflictingKeys.length > 0) {
      const hint = usageHint ? ' ' + usageHint : '';
      throw new Error(
        `Conflicting params: "time" is present, but flat alias${conflictingKeys.length > 1 ? 'es' : ''} ` +
  ...
  if (typeof params.time === 'number') {
    const hint = usageHint ? ' ' + usageHint : '';
    throw new Error(
      'Do not pre-compute epoch timestamps. Pass local datetime strings with timezone. ' +
  ...
  if (at !== undefined) {
    ...
    if (typeof at !== 'string') {
      const hint = usageHint ? ' ' + usageHint : '';
      throw new Error(
        'Malformed time params: at/timestamp aliases must be strings when provided.' +
          hint,
      );
    }
    const time: Record<string, unknown> = toTimePointFromString(String(at));
    if (typeof params.timezone === 'string') {
  ...
  if (params.offset !== undefined) {
    if (typeof params.offset !== 'string') {
      const hint = usageHint ? ' ' + usageHint : '';
      throw new Error(
        'Malformed time params: offset must be a string like "-1h" when provided.' +
          hint,
      );
    }
    ...
    if (!ago) {
      const hint = usageHint ? ' ' + usageHint : '';
      throw new Error(
        `Malformed time params: offset "${params.offset}" is not a supported relative time alias. ` +
  ...
  // Fail closed: if any time-related key was present but no valid shape matched, reject
  const strayTimeKeys = ['time', 'at', 'timestamp', 'timestamp_text', 'timestamp_local', 'timezone', 'timestamp_timezone', 'ago', 'offset'].filter(
    (key) => params[key] !== undefined,
  );
  if (strayTimeKeys.length > 0) {
    const hint = usageHint ? ' ' + usageHint : '';
...
// ---------------------------------------------------------------------------
// Tag extraction (all tag-based skills)
// ---------------------------------------------------------------------------
...
function parseTagPathEntry(
  entry: unknown,
  index: number,
): Record<string, unknown> {
  if (typeof entry !== 'string') {
    throw new Error(
      `tag_paths[${index}] must be a string in "/full/object/path:field" format.`,
    );
  }
  ...
  if (separatorIndex <= 0 || separatorIndex === trimmed.length - 1) {
    throw new Error(
      `tag_paths[${index}] must use "/full/object/path:field" format.`,
    );
  }
  ...
  if (!object.startsWith('/')) {
    throw new Error(
      `tag_paths[${index}] must start with a full object path beginning with /.`,
    );
  }

  if (!field) {
    throw new Error(
      `tag_paths[${index}] must include a non-empty field name after ":".`,
    );
  }
...
export function extractTags(
  ...
  if (Array.isArray(params.tags) && params.tags.length > 0) {
    ...
    return params.tags.map((entry, index) => {
      ...
      if (!isRecord(entry)) {
        throw new Error(
          `tags[${index}] must be an object or a "/full/object/path:field" string.`,
        );
      }
  ...
  if (params.tag !== undefined) {
    const conflictingKeys = ['object', 'objects', 'field', 'label', 'unit', 'functions', 'tag_paths'].filter(
      (key) => params[key] !== undefined,
    );
    if (conflictingKeys.length > 0) {
      throw new Error(
        `Conflicting params: tag is present, but flat alias${conflictingKeys.length > 1 ? 'es' : ''} ` +
          `${conflictingKeys.join(', ')} ${conflictingKeys.length > 1 ? 'are' : 'is'} also present. ` +
    ...
    if (!isRecord(params.tag)) {
      throw new Error(
        'tag must be either a "/full/object/path:field" string or an object entry when provided.',
      );
    }
  ...
  if (params.tag_paths !== undefined) {
    const conflictingKeys = ['object', 'objects', 'field', 'label', 'unit', 'functions'].filter(
      (key) => params[key] !== undefined,
    );
    if (conflictingKeys.length > 0) {
      throw new Error(
        `Conflicting params: tag_paths is present, but flat alias${conflictingKeys.length > 1 ? 'es' : ''} ` +
          `${conflictingKeys.join(', ')} ${conflictingKeys.length > 1 ? 'are' : 'is'} also present. ` +
    ...
    if (!Array.isArray(params.tag_paths) || params.tag_paths.length === 0) {
      throw new Error(
        'tag_paths must be a non-empty array of "/full/object/path:field" strings when provided.',
      );
    }
  ...
  if (params.objects !== undefined) {
    if (!Array.isArray(params.objects) || params.objects.length === 0) {
      throw new Error(
        'objects must be a non-empty array of full object paths when provided.',
      );
    }

    if (params.object !== undefined) {
      throw new Error(
        'Conflicting params: object and objects are both present. Use either a single object or an objects array, not both.',
      );
    }

    if (typeof params.field !== 'string') {
      throw new Error(
        'objects shorthand requires a shared field string (for example { objects: ["/root/..."], field: "out_value" }).',
      );
    }

    return (params.objects as unknown[])
      .map((entry) => {
        if (typeof entry !== 'string') {
          throw new Error(
            'objects must contain only string object paths.',
          );
        }
        ...
        if (typeof params.label === 'string') tag.label = params.label;
        if (typeof params.unit === 'string') tag.unit = params.unit;
