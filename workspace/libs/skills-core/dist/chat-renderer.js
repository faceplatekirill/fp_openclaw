...
function renderHistoryBlock(block, timezone) {
    ...
    if (truncated) {
        sections.push(`Showing ${HISTORY_TABLE_HEAD + HISTORY_TABLE_TAIL} of ${rows.length} points. Truncated to first ${HISTORY_TABLE_HEAD} and last ${HISTORY_TABLE_TAIL}.`);
    }
...
function renderSnapshotBlock(block, timezone) {
...
function renderAggregateRow(row, timezone, functions) {
    return [
...
function renderAggregateTableBlock(block, timezone) {
    const sections = [
        `### ${block.label ?? block.tag}`,
        renderTable(['Period', ...block.functions], block.rows.map((row) => renderAggregateRow(row, timezone, block.functions))),
    ];
...
function renderAlarmEntry(entry, timezone) {
    return [
...
function renderAlarmListBlock(block, timezone) {
    ...
    const sections = [
        '### Alarms',
        renderTable(['Time', 'Object', 'Type', 'Message', 'State'], alarms.map((entry) => renderAlarmEntry(entry, timezone))),
    ];
    if (truncated) {
...
function renderTopOffenders(block) {
...
function renderFloodPeriods(block, timezone) {
    if (block.metrics.flood_periods.length === 0) {
        return '- **Flood periods:** none';
    }
    return `- **Flood periods:** ${block.metrics.flood_periods
        .map((period) => `${formatTimestamp(period.from, timezone)} - ${formatTimestamp(period.to, timezone)} (${period.rate_per_hour}/h)`)
        .join(', ')}`;
}
function renderStanding(block, timezone) {
    if (block.metrics.standing_alarms.length === 0) {
        return '- **Standing alarms:** none';
    }
    return `- **Standing alarms:** ${block.metrics.standing_alarms
        .map((alarm) => `${alarm.source}${alarm.message ? ` (${alarm.message})` : ''} since ${formatTimestamp(alarm.since, timezone)}`)
        .join(', ')}`;
}
function renderChattering(block) {
    ...
    return `- **Chattering alarms:** ${block.metrics.chattering_alarms
        .map((alarm) => alarm.avg_duration_ms === undefined
...
function renderDistribution(title, distribution) {
    ...
    return [
        `#### ${title}`,
        renderTable(['Category', 'Count'], Object.entries(distribution).map(([name, count]) => [name, String(count)])),
    ];
}
function renderAlarmSummaryBlock(block, timezone) {
    const sections = [
        '### Alarm Summary',
        [
    ...
    const prioritySection = renderDistribution('Priority Distribution', block.priority_distribution);
    if (prioritySection.length > 0) {
        sections.push(prioritySection.join('\n\n'));
    }
    const categorySection = renderDistribution('Category Distribution', block.category_distribution);
    if (categorySection.length > 0) {
        sections.push(categorySection.join('\n\n'));
    }
    return sections.join('\n\n');
}
function renderScopeSampleRow(entry) {
    return [entry.name ?? '', entry.pattern ?? '', entry.path];
}
function renderScopeTypeSummary(block) {
    if (block.type_summary && Object.keys(block.type_summary).length > 0) {
        return block.type_summary;
    }
    return block.objects.reduce((acc, entry) => {
        const key = entry.pattern ?? 'unknown';
        acc[key] = (acc[key] ?? 0) + 1;
        return acc;
    }, {});
}
function renderScopeBlock(block) {
    const sections = ['### Scope'];
    if (block.objects.length > SCOPE_GROUP_THRESHOLD) {
        sections.push(renderTable(['Type', 'Count'], Object.entries(renderScopeTypeSummary(block)).map(([type, count]) => [
            type,
            String(count),
        ])));
        sections.push(`Showing sample objects (${Math.min(block.objects.length, SCOPE_SAMPLE_COUNT)} of ${block.total}):`);
        sections.push(renderTable(['Name', 'Type', 'Path'], block.objects
            .slice(0, SCOPE_SAMPLE_COUNT)
            .map((entry) => renderScopeSampleRow(entry))));
        return sections.join('\n\n');
    }
    sections.push(renderTable(['Name', 'Type', 'Path'], block.objects.map((entry) => renderScopeSampleRow(entry))));
    return sections.join('\n\n');
}
function splitCoverageTag(tag) {
...
function renderCoverageNotes(entry) {
...
function renderCoverageBlock(block) {
    const sections = [
        '### Archive Coverage',
        renderTable(['Object', 'Field', 'Status'], block.entries.map((entry) => renderCoverageEntry(entry))),
...
function renderBlock(block, timezone) {
...
function renderCompleteness(viewModel) {
    const { completeness } = viewModel;
    if (completeness.status === 'complete') {
        return null;
    }
    const lines = ['## Completeness'];
    if (completeness.total_available !== undefined &&
        completeness.total_returned !== undefined) {
        lines.push(`Showing ${completeness.total_returned} of ${completeness.total_available} results.`);
    }
...
function renderProvenance(viewModel) {
...
export function renderChatMarkdown(viewModel) {
    const sections = [];
