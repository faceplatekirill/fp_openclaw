...
const RELATIVE_TIME_POINT_UNITS = {
...
function validateRequiredTimezone(timezone, context) {
...
function parseLocalDateTime(input) {
    ...
    if (!match) {
        throw new Error(`Invalid local datetime string: ${input}. Expected YYYY-MM-DD HH:mm or YYYY-MM-DD HH:mm:ss.`);
    }
    const parts = {
        year: Number(match[1]),
    ...
    const validationDate = new Date(Date.UTC(parts.year, parts.month - 1, parts.day, parts.hour, parts.minute, parts.second));
    if (validationDate.getUTCFullYear() !== parts.year ||
...
function resolveLocalDateTimeEpoch(parts, timezone) {
    ...
    if (matches.length === 0) {
        throw new Error(`Local datetime ${parts.year}-${String(parts.month).padStart(2, '0')}-${String(parts.day).padStart(2, '0')} ${String(parts.hour).padStart(2, '0')}:${String(parts.minute).padStart(2, '0')}:${String(parts.second).padStart(2, '0')} does not exist in timezone ${timezone}.`);
    }
...
function labelFromRange(input, timezone) {
    return `${input} (${timezone})`;
}
function validateRelativeTimePointAgo(ago) {
    if (!isRecord(ago)) {
        throw new Error('Relative time point requires ago.amount and ago.unit (for example { ago: { amount: 1, unit: "hour" } }).');
    }
    const amount = typeof ago.amount === 'number' ? ago.amount : Number.NaN;
    if (!Number.isInteger(amount) || amount <= 0) {
        throw new Error(`Relative time point requires a positive integer ago.amount, got: ${String(amount)}`);
    }
    const unit = ago.unit;
    if (typeof unit !== 'string' ||
        !Object.prototype.hasOwnProperty.call(RELATIVE_TIME_POINT_UNITS, unit)) {
        throw new Error(`Relative time point requires ago.unit to be one of ${Object.keys(RELATIVE_TIME_POINT_UNITS).join(', ')}, got: ${String(unit)}`);
    }
    const normalized = RELATIVE_TIME_POINT_UNITS[unit];
...
export function resolveTimeRange(spec, options = {}) {
    const nowMs = resolveNowMs(options.nowMs);
    const resolvedSpec = spec ?? { preset: 'last_1_hour' };
    if ('preset' in resolvedSpec) {
        if (resolvedSpec.preset in ROLLING_PRESETS) {
            const preset = ROLLING_PRESETS[resolvedSpec.preset];
        ...
        const timezone = validateRequiredTimezone(resolvedSpec.timezone, `Calendar preset "${resolvedSpec.preset}"`);
    ...
    const timezone = validateRequiredTimezone(resolvedSpec.timezone, 'Explicit local time range');
...
export function resolveTimePoint(spec, options = {}) {
    const nowMs = resolveNowMs(options.nowMs);
    if ('ago' in spec) {
